...
5. **Locations use HORIZONTAL, characters use VERTICAL** — reference image orientation depends on entity type.
...
def _agents_md_body(skills, title):
    ...
    rows = "\n".join(
        f"| `/fk-{name}` | {desc} |" for name, desc in [(s["name"], s["description"]) for s in skills]
    )
    return (
        f"<!-- AUTO-GENERATED by setup.py — do not edit. Source: skills/ -->\n"
        f"# Flow Kit — {title}\n\n"
