...
def main():
    ...
    parser.add_argument("--scene-count", type=int, default=7)
    ...
    ep_dir = Path(f"output/{args.book_slug}/ep_{args.ep_id:02d}_{title_slug}")
    ep_dir.mkdir(parents=True, exist_ok=True)

    if args.force:
        logger.info("--force: clearing existing artifacts")
        for f in ["script.json", "scene_ids.json", "concat_scenes.mp4", "final.mp4", "caption.txt"]:
    ...
    # Step 6: Music
    logger.info("[6/8] Gen music (skip=%s)", args.skip_music_gen)
    music_src = lib.gen_music(ep_dir, skip_gen=args.skip_music_gen)
    logger.info("    Music: %s", music_src.name)

    # Step 7: Mix
    logger.info("[7/8] Final mix → %s", ep_dir / "final.mp4")
    final = lib.mix_final(concat, tts, music_src, ep_dir, target_dur)

    # Step 8: Caption
    logger.info("[8/8] Gen caption")
