...
async def cancel_batch(project_id: str = None):
    """Cancel all pending/processing requests (optionally for a specific project)."""
    rows = await crud.list_requests(project_id=project_id)
    count = 0
    for r in rows:
        if r.get("status") in ("PENDING", "PROCESSING"):
            await crud.update_request(r["id"], status="FAILED", error_message="Canceled by user")
            count += 1
