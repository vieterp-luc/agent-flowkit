...
# ─── Backend 2: LLM SDK (routed via mz-9router) ──────────────
...
async def _analyze_sdk(
    ...
    """Send individual frames to Vision via OpenAI SDK (to mz-9router)."""
    import os
    from openai import AsyncOpenAI
    from agent.api.models import get_default_chat_model
    
    # Connect to 9router proxy
    client = AsyncOpenAI(
    ...
    for char in characters:
        ...
        if char.get("reference_image_url") and ((slug and slug in character_names) or (name and name in character_names)):
            ...
            content.append({"type": "image_url", "image_url": {"url": char["reference_image_url"], "detail": "low"}})
    if content:
        content.append({"type": "text", "text": "Video frames to analyze:"})

    for frame_path in frames:
        b64_data = _frame_to_base64(frame_path)
        content.append({
            "type": "image_url",
            "image_url": {"url": f"data:image/jpeg;base64,{b64_data}", "detail": "low"},
        })
