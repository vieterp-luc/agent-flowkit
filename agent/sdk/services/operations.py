...
def _save_raw_bytes(
    ...
    for op in operations:
        ...
        try:
            ...
            path = scene_4k_path(project_slug, scene_id)
...
class OperationService:
    ...
    async def generate_scene_video(self, scene: dict, orientation: str,
        ...
        if request_id:
            ...
            if req_row:
                ...
                err_msg = req_row.get("error_message", "") or ""
                if "AUDIO_FILTERED" in err_msg.upper():
                    strip_audio = True

        # Check ALL historical requests for this scene to catch manual retries
        if not strip_audio and scene.get("id"):
            prev_reqs = await crud.list_requests(scene_id=scene.get("id"))
            for pr in prev_reqs:
                if pr.get("type") in ("GENERATE_VIDEO", "REGENERATE_VIDEO") and "AUDIO_FILTERED" in (pr.get("error_message") or "").upper():
                    strip_audio = True
                    break

        if strip_audio:
            logger.warning("AUDIO_FILTERED detected in previous attempts, stripping audio instructions")
    ...
    async def generate_scene_video_refs(self, scene: dict, orientation: str,
        ...
        # Check ALL historical requests for this scene
        if not strip_audio and scene.get("id"):
            prev_reqs = await crud.list_requests(scene_id=scene.get("id"))
            for pr in prev_reqs:
                if pr.get("type") in ("GENERATE_VIDEO", "REGENERATE_VIDEO") and "AUDIO_FILTERED" in (pr.get("error_message") or "").upper():
        ...
        if strip_audio:
            logger.warning("AUDIO_FILTERED detected in previous attempts, stripping audio instructions for r2v")
...
async def _build_video_prompt(base_prompt: str, scene: dict, project_id: str | None, strip_audio: bool = False) -> str:
