...
_VALID_TABLES = frozenset({"character", "project", "video", "scene", "request", "material", "chat_message", "chat_session"})
...
_COLUMNS = {
    ...
    "material": {"name", "style_instruction", "negative_prompt", "scene_prefix", "lighting"},
    "chat_message": {"project_id", "role", "content"},
}
...
async def create_video(project_id: str, title: str, video_story: str = None, display_order: int = 0, orientation: str = None) -> dict:
    db = await get_db()
    vid, now = _uuid(), _now()
    async with _db_lock:
        await db.execute(
            "INSERT INTO video (id,project_id,title,video_story,display_order,orientation,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)",
            (vid, project_id, title, video_story, display_order, orientation, now, now))
        await db.commit()
...
async def create_scene(video_id: str, display_order: int, prompt: str,
                       ...
                       source: str = "root", narrator_text: str = None) -> dict:
    db = await get_db()
    sid, now = _uuid(), _now()
    chars_json = json.dumps(character_names) if character_names is not None else None
    async with _db_lock:
        await db.execute(
...
async def create_request(req_type: str, orientation: str = None,
                         ...
                         source_media_id: str = None, retry_count: int = 0, **_kw) -> dict:
    db = await get_db()
    rid, now = _uuid(), _now()
    async with _db_lock:
        await db.execute(
            """INSERT INTO request (id,project_id,video_id,scene_id,character_id,type,orientation,source_media_id,retry_count,created_at,updated_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (rid, project_id, video_id, scene_id, character_id, req_type, orientation, source_media_id, retry_count, now, now))
        await db.commit()
...
async def update_material(mid: str, **kw): return await _update("material", "id", mid, **kw)
...
async def create_chat_session(project_id: str, title: str = "New Chat") -> dict:
    db = await get_db()
    sid, now = _uuid(), _now()
    async with _db_lock:
        await db.execute(
            "INSERT INTO chat_session (id,project_id,title,created_at) VALUES (?,?,?,?)",
            (sid, project_id, title, now))
        await db.commit()
    return await _get_with_db(db, "chat_session", "id", sid)

async def list_chat_sessions(project_id: str) -> list[dict]:
    db = await get_db()
    cur = await db.execute("SELECT * FROM chat_session WHERE project_id=? ORDER BY created_at DESC", (project_id,))
    return [dict(r) for r in await cur.fetchall()]

async def create_chat_message(project_id: str, session_id: str, role: str, content: str) -> dict:
    db = await get_db()
    mid, now = _uuid(), _now()
    async with _db_lock:
        await db.execute(
            "INSERT INTO chat_message (id,project_id,session_id,role,content,created_at) VALUES (?,?,?,?,?,?)",
            (mid, project_id, session_id, role, content, now))
        await db.commit()
    return await _get_with_db(db, "chat_message", "id", mid)

async def list_chat_messages(session_id: str, limit: int = 50) -> list[dict]:
    db = await get_db()
    # Fetch latest `limit` messages, but return them in ascending chronological order
    cur = await db.execute(
        "SELECT * FROM (SELECT * FROM chat_message WHERE session_id=? ORDER BY created_at DESC LIMIT ?) ORDER BY created_at ASC",
        (session_id, limit)
    )
    return [dict(r) for r in await cur.fetchall()]
