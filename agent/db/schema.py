...
CREATE TABLE IF NOT EXISTS project (
    ...
    orientation TEXT DEFAULT 'HORIZONTAL' CHECK(orientation IN ('VERTICAL','HORIZONTAL')),
    created_at  TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
    updated_at  TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))
);
...
""")
        ...
        # Migration: rename description to video_story (or story to video_story if we ran the previous one)
        if "description" in video_columns and "video_story" not in video_columns:
            await db.execute("ALTER TABLE video RENAME COLUMN description TO video_story")
            logger.info("Migrated: renamed description -> video_story in video table")
        elif "story" in video_columns and "video_story" not in video_columns:
            await db.execute("ALTER TABLE video RENAME COLUMN story TO video_story")
            logger.info("Migrated: renamed story -> video_story in video table")
    ...
    created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')))""")
        ...
        # Migration: create chat_session table if missing
        cursor = await db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='chat_session'")
        if not await cursor.fetchone():
            await db.execute("""CREATE TABLE chat_session (
    id TEXT PRIMARY KEY, project_id TEXT NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    title TEXT, created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')))""")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_chat_session_project ON chat_session(project_id)")
            logger.info("Migrated: created chat_session table")

        # Migration: create chat_message table if missing
        cursor = await db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='chat_message'")
        if not await cursor.fetchone():
            await db.execute("""CREATE TABLE chat_message (
    id TEXT PRIMARY KEY, project_id TEXT NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    session_id TEXT REFERENCES chat_session(id) ON DELETE CASCADE,
    role TEXT NOT NULL CHECK(role IN ('user','assistant','system')), content TEXT NOT NULL,
    created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')))""")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_chat_message_project ON chat_message(project_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_chat_message_session ON chat_message(session_id)")
            logger.info("Migrated: created chat_message table")
        else:
            # Migration: add session_id to chat_message if missing
            cursor = await db.execute("PRAGMA table_info(chat_message)")
            columns = {row[1] for row in await cursor.fetchall()}
