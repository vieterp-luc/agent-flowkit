...
Ask the user for:
1. **Project name** (Channel Name) and **story** (Concept/Summary of the channel or first episode).
2. **Material** — the visual style for all images in this channel. Choose one of the 6 built-in styles or a custom material. Run `GET /api/materials` to show available options. Built-ins: `realistic`, `3d_pixar`, `anime`, `stop_motion`, `minecraft`, `oil_painting`. **Required.**
3. **Configuration parameters** — ask the user for generation limits if they want to override defaults:
   - **Assets**: Minimum and maximum number of characters, locations, and visual assets to extract.
   - **Videos**: Number of videos to generate from the story.
   - **Scenes**: Number of scenes per video.
4. **Characters** — name + visual description of their **base default look in ONE outfit only**. (Follow user's min/max limits). No scene-specific variants (e.g. "glamorous in studio, sporty in gym"). The reference image will be generated as a comprehensive multi-panel character sheet (Body shots, Turnaround, Expression sheet, and Pose sheet). Different outfits per scene come from the scene prompts, not the character description.
5. **Locations** — name + visual description of key places (Follow user's min/max limits).
6. **Visual assets** — name + visual description of key props/objects (Follow user's min/max limits).
7. **Number of scenes** (for Episode 1) and **orientation** (VERTICAL or HORIZONTAL).
...
video_prompt: "Medium shot of The Commander seen from behind at a podium, hand gesturing firmly at the large screen showing a regional map, advisors seated facing him. The camera slowly pushes forward. Dramatic overhead lighting casts deep shadows, cool blue glow from the screen illuminating his silhouette. Broad shoulders tensing as markers appear one by one on the screen. Warm overhead rim light contrasts with the cool blue screen glow, dark room atmosphere.\n\nAudio: quiet hum of electronics, air conditioning.\nSFX: map markers clicking into place, pen tapping on table, muffled breathing.\nNegative: subtitles, watermark, text overlay, blurry faces."
```
...
curl -X POST http://127.0.0.1:8100/api/videos \
  -H "Content-Type: application/json" \
  -d '{"project_id": "<PID>", "title": "...", "video_story": "...", "display_order": 0, "orientation": "VERTICAL|HORIZONTAL"}'
```
...
Scene 4 (soldiers drinking in barracks):
  video_prompt: "Medium wide shot of soldiers gathered around a rough wooden table,
    ...
    toward his face. Warm candlelight, golden tones deepening to shadow.

    Audio: muffled laughter, crackling fire.
    SFX: tin cups clinking, liquid sloshing.
    Negative: subtitles, watermark, text overlay."

  transition_prompt: "Medium shot of the Defector raising his cup among laughing
    soldiers in the warm barracks interior. His expression shifts — he puts down 
    the cup, glances toward the barracks door, jaw tightening. He rises abruptly,
...
Scene 5 (Defector bursting through door — CHILD of scene 4):
  ...
  video_prompt: "Medium shot of the Defector bursting through the barracks door,
    stumbling into cold night air, breaking into a run toward darkness.
    The camera follows with handheld movement, slight shake. Cold blue moonlight, 
    his silhouette against the snow, breath visible in the freezing cold.

    Audio: cold wind howling, distant dogs barking.
    SFX: door slamming shut, boots crunching on frozen ground, heavy breathing.
    Negative: subtitles, watermark, text overlay."
```
...
Write video prompts as **natural prose** — like briefing a film director. Veo 3 generates native audio (SFX, ambient) from text.
...
- **Strictly single-shot** — use one continuous camera action for the 8s clip, no "cut to" or multiple shots.
- **No character dialogue** — exclude all speech. Only describe environmental sound and SFX.
- Every prompt needs: lighting description + audio description

**Emotional arc pattern (map to 8s):**
Focus the 8-second action on a single continuous motion or emotional shift:
```
Opening (0-2s): Establish the subject and environment
Rising  (2-5s): Subject performs the core action, camera continues moving
Peak    (5-8s): Action concludes or reaches tension point
```
...
across the ground. The camera slowly dollies in as she takes her first
steps on the candy ground, looking around in wonder at the vast landscape. 
Warm golden hour light, soft pastel tones.

Audio: gentle warm breeze, faint magical shimmer.
SFX: metal hatch opening, soft footsteps on crystallized sugar ground.
Negative: subtitles, watermark, text overlay.
```
...
curl -X PATCH http://127.0.0.1:8100/api/scenes/<SID> \
  ...
  -d '{
    ...
    "video_prompt": "Wide shot of Hero sprinting across the Castle bridge at dawn, golden light catching the blade of Magic Sword raised high. The camera tracks alongside with steady movement. Warm golden hour light, long shadows stretching across ancient stone, wind whipping his cloak. His face is determined, Castle gate looming behind, warm side light.\n\nAudio: wind across stone bridge, distant morning birds.\nSFX: boots pounding on stone, sword ringing, cloak snapping.\nNegative: subtitles, watermark, text overlay.",
