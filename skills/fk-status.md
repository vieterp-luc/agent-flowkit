...
Usage: `/fk-status <project_id> [video_id]` or `/fk-status` (lists all projects)
...
### 5. Videos Summary
If no `video_id` is provided, print a summary table of all videos in the channel:
| ID | Video Title | Scenes | Image Done | Video Done | Upscale Done |
|----|-------------|--------|------------|------------|--------------|

*(You will need to fetch scenes for each video to calculate these counts, or infer from the video status if available).*

### 6. Specific Video Scenes (If `video_id` is provided)
If a specific `video_id` is passed (e.g. `/fk-status <PID> <VID>`), fetch its scenes:
```bash
...
### 7. Pending/processing requests
```bash
curl -s http://127.0.0.1:8100/api/requests/pending
```

### 8. Summary
If focusing on a single video, print orientation and counts: `Orientation: ${ORI}` then X/Y refs ready, X/Y images done, X/Y videos done, X/Y upscaled.
