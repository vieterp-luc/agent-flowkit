...
Usage: `/fk-pipeline [project_id] [video_id] [options]`
...
Examples:
- `/fk-pipeline` — detect state and continue most recent video of most recent project
- `/fk-pipeline <project_id>` — prompt user to select a video if multiple exist
- `/fk-pipeline <project_id> <video_id> --upscale --tts --download` — full run on specific video
- `/fk-pipeline <project_id> <video_id> HORIZONTAL --upscale` — explicit video + orientation
...
# Get video + scenes
# If video_id is omitted, fetch videos and select the latest OR ask the user if multiple exist:
curl -s "http://127.0.0.1:8100/api/videos?project_id=<PID>"
# Select the target <VID>
