...
👉 **[View the full breakdown of all stages for each skill here (SKILLS_STAGES.md)](SKILLS_STAGES.md)**

## Pipeline (run in order)

| # | Skill | File | Description |
|---|-------|------|-------------|
| 1 | `fk-create-project` | [fk-create-project.md](fk-create-project.md) | Create channel universe + entities + first episode |
| 1a | `fk-add-video` | [fk-add-video.md](fk-add-video.md) | Create a new episode in an existing channel |
| 1b | `fk-add-entity` | [fk-add-entity.md](fk-add-entity.md) | Add new character/location to an existing channel |
...
# Create 3 scenes
# Scene 1: prompt (image) + video_prompt (8s single shot)
curl -X POST http://127.0.0.1:8100/api/scenes -H "Content-Type: application/json" \
  -d '{
    ...
    "video_prompt": "Wide crane down shot, Luna emerges from rocket hatch onto Candy Planet Surface, looking around in wonder at the candy ground. The camera slowly dollies in. Wide establishing shot of the vast landscape with cotton candy clouds. Silence, gentle wind. Audio: gentle wind, distant spaceship hum. SFX: metal hatch opening, soft footsteps. Negative: subtitles, watermark, text overlay.",
...
curl -X POST http://127.0.0.1:8100/api/scenes -H "Content-Type: application/json" \
  -d '{
    ...
    "video_prompt": "Over-the-shoulder shot, Luna kneels at Chocolate River edge on Candy Planet Surface, dipping a paw into the chocolate with a surprised expression. The camera tracks alongside her. Warm lighting reflected off chocolate, steam rising. Audio: river flowing, warm breeze. SFX: chocolate dripping, soft rustling. Negative: subtitles, watermark, text overlay.",
...
curl -X POST http://127.0.0.1:8100/api/scenes -H "Content-Type: application/json" \
  -d '{
    ...
    "video_prompt": "Low angle push in, Luna plants a small flag on top of Gummy Bear Mountain with a triumphant pose. The camera slowly arcs around her. Epic bird eye view, Candy Planet Surface stretching to the horizon below, golden sunset glowing through translucent gummy bears. Audio: high altitude wind howling. SFX: flag snapping in wind, footsteps on gummy surface. Negative: subtitles, watermark, text overlay.",
